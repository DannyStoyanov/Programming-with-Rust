...
impl Add for Bigint {
    ...
    fn add(self, other: Self) -> Self {
        if self.sign == other.sign {
            return Bigint { sign: self.sign, digits: add_vectors(&self.digits, &other.digits) }
        }
        else {
            match direct_compare(&self.digits, &other.digits) {
                1 => return Bigint { sign: self.sign, digits: filter_digits(&sub_vectors(&self.digits, &other.digits)) },
               -1 => return Bigint { sign: other.sign, digits: filter_digits(&sub_vectors(&other.digits, &self.digits)) },
                0 => return Bigint { sign: 1, digits: vec![0] },
                _ => return Bigint { sign: 1, digits: vec![0] },
            }
        }
    }
}
...
impl Sub for Bigint {
    ...
    fn sub(self, other: Self) -> Self {
        ...
        let temp_number = Bigint { sign: res_sign, digits: other.digits};
        if self.sign == temp_number.sign {
            return Bigint { sign: self.sign, digits: add_vectors(&self.digits, &temp_number.digits) }
        }
        else {
            match direct_compare(&self.digits, &temp_number.digits) {
                1 => return Bigint { sign: self.sign, digits: filter_digits(&sub_vectors(&self.digits, &temp_number.digits)) },
               -1 => return Bigint { sign: temp_number.sign, digits: filter_digits(&sub_vectors(&temp_number.digits, &self.digits)) },
                0 => return Bigint { sign: 1, digits: vec![0] },
                _ => return Bigint { sign: 1, digits: vec![0] },
            }        
        }
...
fn main() {
    let v1 = vec![8,6,4,2,3];
    let v2 = vec![4,2,3];
    println!("------------- PRINT TESTS -------------");
    println!("{:?}", bigint("543") - bigint("1"));
}
